...
userSchema.methods.toggleFavorite = function (itemId) {
  const index = this.favoriteList.findIndex(fav => fav.toString() === itemId.toString());
  if (index === -1) {
...
userSchema.methods.toggleComment = function (commentId) {
  const index = this.comments.findIndex(comment => comment.toString() === commentId.toString());
  if (index === -1) {
